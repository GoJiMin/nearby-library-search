...
- [ ] 세그먼트 탭과 단일 입력 필드 기반 검색 시작 화면을 구현한다.
- [ ] 홈 보조 설명 영역과 예시 검색어를 Phase 4-2 계약에 맞게 구현한다.
- [ ] 홈 화면의 로딩/오류/빈 상태와 첫 행동 UX를 검증한다.
- [ ] Phase 5-1 내용을 기준으로 `spec.md`와 `task.md`를 작성한다.

## Phase 5-2. 도서 검색 결과와 도서 선택 구현

- [ ] 텍스트 우선 카드 구조의 도서 검색 결과 화면을 구현한다.
- [ ] 카드별 즉시 선택 CTA와 재검색 중심 empty/error 흐름을 구현한다.
- [ ] 도서 검색 결과와 선택 상태를 Phase 4-2 계약에 맞게 검증한다.
- [ ] Phase 5-2 내용을 기준으로 `spec.md`와 `task.md`를 작성한다.

## Phase 5-3. 지역 선택 다이얼로그 구현

- [ ] 상위 지역 필수, 세부 지역 선택적 refinement 구조의 지역 선택 다이얼로그를 구현한다.
- [ ] `전체` 기본값, 선택 요약, CTA 활성 조건, fallback 규칙을 구현한다.
- [ ] 키보드/포커스/비활성 설명 기준을 포함해 지역 선택 흐름을 검증한다.
- [ ] Phase 5-3 내용을 기준으로 `spec.md`와 `task.md`를 작성한다.

## Phase 5-4. 도서관 결과 화면 구현

- [ ] 데스크톱 3영역 구조와 모바일 정보 우선 구조의 결과 화면을 구현한다.
- [ ] 리스트, 지도 마커, 하단 상세 정보 패널의 선택 상태 동기화를 구현한다.
- [ ] 카카오맵을 연결해 전체 마커 표시와 선택 강조를 구현한다.
- [ ] Phase 5-4 내용을 기준으로 `spec.md`와 `task.md`를 작성한다.
