...
- [ ] 도서 검색 결과 카드의 `상세 보기` 버튼을 실제 도서 상세 조회 다이얼로그로 연결한다.
- [ ] 도서 기본 정보, 표지, 대출 정보, 상세 링크의 표시 규칙을 현재 contracts와 UX 계약에 맞게 정리한다.
- [ ] 상세 보기의 loading, empty, error, close 흐름을 결과 화면 안에서 자연스럽게 처리한다.
