...
function createRetryableUpstreamRequestError(
  title: ErrorResponse['title'],
  resourceName: string,
) {
  return createErrorResponse(
    title,
    `${resourceName} 정보를 불러오지 못했습니다. ${RETRYABLE_ERROR_SUFFIX}`,
    502,
  )
}

function createRetryableUpstreamResponseError(
  title: ErrorResponse['title'],
  resourceName: string,
) {
  return createErrorResponse(
...
function toLibraryApiErrorResponse(
  error: unknown,
