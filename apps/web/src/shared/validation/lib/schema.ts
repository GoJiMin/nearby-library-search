...
function createPositiveIntegerSchema(defaultValue: number, maxValue?: number) {
  const numberSchema = maxValue === undefined ? z.number().int().min(1) : z.number().int().min(1).max(maxValue);
...
export {createOptionalTrimmedStringSchema, createPositiveIntegerSchema, normalizeOptionalString};
