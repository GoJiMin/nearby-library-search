...
const detailSectionLabelClassName = 'text-text-muted text-xs leading-none font-semibold tracking-[0.16em] uppercase';
...
function BookDetailDialogResolvedContent({isbn13}: BookDetailDialogResolvedContentProps) {
  ...
  return (
    <div className="grid h-full min-h-0 grid-cols-1 lg:grid-cols-[minmax(0,20rem)_minmax(0,1fr)]">
      <aside className="bg-surface-muted/35 px-6 py-8 lg:px-8 lg:py-10">
          ...
          <div className="flex flex-1 items-center justify-center lg:items-start">
            {book.imageUrl ? (
              <img
                ...
                className="aspect-[3/4] w-full max-w-60 rounded-3xl object-cover sm:max-w-64 lg:max-w-72"
                src={book.imageUrl}
              />
            ) : (
              <div className="bg-surface border-line flex aspect-[3/4] w-full max-w-60 items-center justify-center rounded-3xl border sm:max-w-64 lg:max-w-72">
