...
- [ ] 서비스의 주 목표와 주 사용자를 설명한다.
- [ ] 홈 검색 시작부터 도서 상세 보기까지 핵심 사용자 플로우를 순서대로 정리한다.
- [ ] 현재 MVP에서 사용자가 실제로 할 수 있는 핵심 행동을 정리한다.
- [ ] `web + bff + contracts` 구조와 BFF 도입 이유를 API 키 보호, provider boundary, 응답 정규화 기준으로 설명한다.
