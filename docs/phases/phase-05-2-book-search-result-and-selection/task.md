...
- [x] 스크린샷 기준의 압축 검색 바 구조를 구현한다.
- [x] `책 제목` / `저자명` 탭, 넓은 입력, 검색 아이콘 제출 affordance, 글자 수 표시를 결과 화면용으로 재구성한다.
- [x] 현재 URL 검색 상태를 입력 초기값으로 반영하고, 재검색 제출 시 `page=1`로 리셋되도록 연결한다.
- [x] 탭 전환, 모드별 마지막 입력값 유지, 재검색 제출을 integration test로 검증한다.

## 4. 검색 결과 요약과 리스트 구조 구현

- [x] `"검색어"에 대한 N개의 검색 결과가 있습니다.` 형태의 동적 결과 요약 제목을 구현한다.
- [x] `useGetSearchBooks`를 연결해 단일 컬럼 결과 리스트를 렌더링한다.
- [x] 홈 히어로 대신 결과 화면 레이아웃이 보이도록 결과 모드 화면 구조를 고정한다.
- [x] 결과 화면 진입과 요약 영역 렌더링을 integration test로 검증한다.
...
- [x] 카드마다 `상세 보기`, `소장 도서관 찾기` 버튼을 구현한다.
- [x] `상세 보기`는 후속 상세 다이얼로그용 `isbn13` handoff만 만들고 실제 다이얼로그는 구현하지 않는다.
- [x] `소장 도서관 찾기`는 후속 지역 선택 다이얼로그용 책 선택 handoff만 만들고 실제 다이얼로그는 구현하지 않는다.
- [x] 1개 결과여도 자동 진행하지 않고 버튼 클릭이 필요하도록 고정한다.
- [x] 버튼별 handoff payload를 integration test로 검증한다.
...
- [x] loading 상태에서 상단 검색 바와 결과 요약은 유지하고 카드 스켈레톤을 렌더링한다.
- [x] empty 상태에서 검색 모드와 검색어를 유지한 채 같은 화면에서 재검색 가능하도록 구현한다.
- [x] error 상태에서 인라인 복구 UI와 다시 시도 흐름을 제공한다.
