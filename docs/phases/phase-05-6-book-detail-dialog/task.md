...
- [ ] `/api/books/:isbn13` normalized response에서 `loanInfo.total`만 대표 값으로 남긴다.
- [ ] `loanInfo.byAge`만 정규화하고 `genderResult`, `regionResult`는 버린다.
- [ ] `book === null`을 error가 아니라 empty 가능한 정상 응답으로 유지한다.
- [ ] 기존 error helper와 route 구조를 그대로 유지한다.

## 3. BFF 상세 route 회귀 검증 정리

- [ ] `/api/books/:isbn13` integration test가 축소된 `loanInfo` shape를 검증하도록 갱신한다.
- [ ] `byGender`, `byRegion`가 normalized response에 남지 않는지 검증한다.
- [ ] upstream payload 일부 누락 시에도 `loanInfo.total + byAge`가 안전하게 정규화되는지 검증한다.
