...
- [x] 결과 카드가 store action을 직접 사용하도록 연결한다.
...
- [x] 제목, 저자, 표지, 출판사/출판일 또는 출판연도, ISBN/ISBN13, 분류 정보를 렌더한다.
- [x] 값이 없는 필드는 가짜 문구 없이 숨긴다.
- [x] `description`이 있을 때만 설명 섹션을 렌더한다.
- [x] desktop은 정보형 dialog, mobile은 단일 컬럼 스크롤 구조로 구현한다.
...
- [x] 결과 카드에서 `상세 보기`를 눌러 상세 창을 열 수 있는지 검증한다.
- [x] loading, success, empty, error를 사용자 기능 기준으로 검증한다.
- [x] 제목, 저자, 표지, 메타 정보, 전체 대출 정보, 연령별 대출 정보를 확인할 수 있는지 검증한다.
- [x] 설명이 없으면 보이지 않는지 검증한다.
- [x] 창을 닫으면 결과 화면으로 자연스럽게 돌아가는지 검증한다.
- [x] 검색 조건이 바뀌면 상세 창이 닫히는지 검증한다.
