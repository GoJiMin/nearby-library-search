...
- [x] `available`, `unavailable`, `error`, `not-owned` 시나리오를 모두 제공한다.
...
- [x] availability 상태를 zustand에 올리지 않는 규칙을 유지한다.

## 10. desktop detail CTA 연결

- [x] desktop detail panel의 `대출 가능 여부 조회` 버튼을 실제 query에 연결한다.
- [x] pending 시 버튼 내부 spinner와 disabled를 적용한다.
- [x] success 시 `대출이 가능해요` / `대출이 불가능해요` / `소장하지 않아요`를 표시한다.
- [x] error 시 버튼 문구를 `재시도`로 바꾸고, toast로 실패를 안내한다.

## 11. mobile detail CTA 연결

- [x] mobile detail 영역의 `대출 가능 여부 조회` 버튼에 같은 상태 모델을 연결한다.
- [x] quick map 동작과 availability 상태가 섞이지 않게 분리한다.
- [x] desktop과 동일한 버튼/문구 계약을 적용한다.
- [x] mobile details fallback/로딩 상태와 availability CTA 배치가 충돌하지 않는지 확인한다.
