...
- [ ] `requestLibraryApi` endpoint union에 `/bookExist`를 추가한다.
- [ ] `libraryCode`, `isbn13` path param 검증 schema를 추가한다.
...
- [ ] `/bookExist` raw envelope를 `LibraryAvailabilityResponse`로 정규화하는 helper를 구현한다.
...
- [ ] `libraryAvailabilityFixture.data.ts`를 추가한다.
...
- [ ] `createApp().inject()` 기준 success 응답을 검증한다.
- [ ] fixture mode 응답과 외부 호출 미발생을 검증한다.
...
- [ ] `librariesQueryKeys.availability`를 추가한다.
