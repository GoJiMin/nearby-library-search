...
- [x] 좌측 `시/도`, 우측 `세부 지역` 2열 레이아웃을 구현한다.
- [x] 좌측은 `REGION_OPTIONS`, 우측은 선택된 상위 지역 기준 `DETAIL_REGION_OPTIONS_BY_REGION`로 렌더링한다.
- [x] 첫 진입에 상위 지역이 없으면 세부 지역 목록은 비활성 상태로 렌더링한다.
- [x] 상위 지역 선택 후 세부 지역의 기본값은 항상 `전체`가 되도록 구현한다.
- [x] 상위 지역 변경 시 기존 세부 지역 선택이 새 지역의 `전체`로 리셋되는지 integration test로 검증한다.
...
- [x] `선택 완료` 후 dialog를 닫고 마지막 확정 지역 선택을 page state에 저장한다.
- [x] dialog를 다시 열면 마지막 확정 선택이 draft로 복원되게 한다.
- [x] confirm 없이 닫은 변경은 버려지고, 재오픈 시 마지막 확정 선택으로 돌아가는지 검증한다.
- [x] `/books` 결과 화면에서 다른 책을 눌러도 마지막 지역 선택이 유지되는 현재 정책을 integration test로 잠근다.
