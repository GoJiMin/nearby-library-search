...
- [ ] `책 제목` / `저자명` 세그먼트 탭 UI를 구현한다.
- [ ] 현재 선택된 검색 모드가 시각적으로 드러나도록 구현한다.
- [ ] 세그먼트 전환 시 입력값이 유지되도록 동작을 고정한다.
- [ ] 탭 전환과 입력 유지 동작을 integration test로 검증한다.
...
- [ ] 짧은 안내 1문단을 Phase 4-2 계약에 맞게 구현한다.
- [ ] 현재 검색 모드에 맞는 예시 검색어 2~4개를 구현한다.
- [ ] 예시 검색어 클릭 시 입력값만 채우고 자동 제출은 하지 않도록 구현한다.
- [ ] 예시 검색어가 별도 카드 섹션으로 확장되지 않도록 구조를 제한한다.
- [ ] 예시 검색어 상호작용을 integration test로 검증한다.
...
- [ ] 홈 화면 제목이 실제 `h1` semantics를 유지하는지 확인한다.
- [ ] 검색 입력에 명시적 라벨이 연결되는지 확인한다.
- [ ] 세그먼트 탭과 CTA가 키보드 탭 순서대로 접근 가능한지 확인한다.
- [ ] 포커스 링과 helper text 위치가 Phase 4-2 접근성 계약과 일치하는지 확인한다.
- [ ] 위 접근성 요구사항을 integration test assertion에 포함한다.
